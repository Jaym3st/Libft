# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: jaisanch <jaisanch@student.42.fr>          +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2022/09/15 15:59:58 by jaimesanche       #+#    #+#              #
#    Updated: 2022/12/17 15:40:08 by jaisanch         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

NAME	= libft.a

CC	= gcc

FLAGS = -Wall -Wextra -Werror

AR = ar rcs

RM	= rm -rf

FILES	= ft_bzero.c ft_memcpy.c ft_memcpy.c ft_memmove.c ft_memchr.c \
		ft_memcmp.c ft_memset.c ft_strlen.c ft_strlcpy.c ft_strlcat.c \
		ft_strchr.c \
		ft_strrchr.c ft_strnstr.c ft_strncmp.c ft_atoi.c ft_isalpha.c\
		ft_isdigit.c ft_isalnum.c ft_isascii.c ft_isprint.c ft_toupper.c \
		ft_tolower.c ft_calloc.c ft_strdup.c ft_substr.c ft_strmapi.c\
		ft_strjoin.c ft_strtrim.c ft_itoa.c ft_strchr.c ft_putchar_fd.c \
		ft_putendl_fd.c ft_putnbr_fd.c ft_putstr_fd.c ft_split.c ft_striteri.c \

BONUS =	ft_lstnew_bonus.c ft_lstadd_front_bonus.c ft_lstsize_bonus.c ft_lstlast_bonus.c ft_lstadd_back_bonus.c \
		ft_lstdelone_bonus.c ft_lstiter_bonus.c ft_lstclear_bonus.c ft_lstmap_bonus.c\

BONUS_OBJS		= $(BONUS:.c=.o)
OBJS	= ${FILES:.c=.o}

.c.o:
	${CC} ${FLAGS} -c $< -o ${<:.c=.o}

$(NAME):	$(OBJS)
		ar rcs $(NAME) $(OBJS)

all:	${NAME}

clean:
	${RM} ${OBJS} $(BONUS_OBJS)
	
fclean: clean
	${RM} ${NAME}

bonus:	$(OBJS) $(BONUS_OBJS)
		@make OBJS="$(OBJS) $(BONUS_OBJS)" 

re: fclean all

.PHONY:	all clean fclean re bonus